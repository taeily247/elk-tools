...
-r, --remove              : Remove  ELK
-u, --user [ STRING ]     : ELK User (deafult: app)
-s, --svr  [ STRING ]     : ELK Service name (deafult: elk-test.com)
-p, --path [ STRING ]     : ELK Path (deafult: /DATA)
-v, --ver  [  INT   ]     : ELK Version (deafult: 9.0.3)
--min-mem  [ INT Gb ]     : JVM Heap Minimum size (default: 6g)
--max-mem  [ INT Gb ]     : JVM Heap Maximum size (default: 6g)
EOF
    exit 0
}

function setOptions() {
    arguments=$(getopt --options u:s:p:v:i:r:h \
    --longoptions user:,svr:,path:,ver:,min-mem:,max-mem:,install:,remove:,help \
    --name $(basename $0) \
    -- "$@")
...
elasticsearch.url: \"http://${ES_SVR}:9200\"
EOF"
    else
...
EOF"
    ...
    if ! $(grep -q "${ELK_PATH}/logstash" ${ELK_PATH}/logstasth/config/startup.options); then
...
\"restart\" )
    if [ -e \\\${LOGSTASH_PID} ]; then
        ...
        if [ ! -f ${LOGSTASH_PID} ]; then
            ...
            echo \"\\\${LOGSTASH_PROC} -d -p \\\${LOGSTASH_PID} &\"
            \\\${LOGSTASH_PROC} -d -p \\\${LOGSTASH_PID} &
            exit 0
        fi
        echo \"Stop Failed, please check if Logstash was already stopped.\"
        exit 0
